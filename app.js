...
async function startServer(portToUse = currentPort) {
  // 1. Connect to Database (Atlas or Dev fallback)
  const conn = await connectDB();

  // 2. View Engine Setup (EJS + SSR Layouts)
  ...
  // 10. Start Listening with auto-retry on address in use
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      ...
      console.log(`⚠️  Port ${portToUse} is in use, auto-retrying on port ${nextPort}...`);
